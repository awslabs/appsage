...
{
    internal class SampleGraphFilter1
    {
        // This is the extracted code from QueryTool.MyQuery method for testing
        public static string GetDynamicFilterCode()
        {
            return @"
using System;
...
public static class CustomGraphFilter1 
{
    ...
    private static double CalculateBetweennessCentrality(Node node, DirectedGraph graph)
    {
    ...
    private static double CalculateComplexityScore(Node node, List<Edge> incomingEdges, List<Edge> outgoingEdges)
    {
        ...
        // Additional test methods for different query examples
        public static string GetSimpleProjectFilterCode()
        {
            return @"sourceGraph.Nodes.Where(n => n.Type == ""Project"")";
        }

        public static string GetComplexClassFilterCode()
        {
            return @"sourceGraph.Nodes.Where(n => 
                n.Type == ""CLASS"" && 
                n.Attributes.ContainsKey(""ClassMethodCount"") && 
                int.Parse(n.Attributes[""ClassMethodCount""]) > 10)";
        }

        public static string GetRepositoryFilterCode()
        {
            return @"sourceGraph.Nodes.Where(n => 
                n.Attributes.ContainsKey(""RepositoryName"") && 
                n.Attributes[""RepositoryName""].Contains(""Core""))";
        }

        public static string GetInheritanceEdgeFilterCode()
        {
            return @"sourceGraph.Edges.Where(e => e.Type == ""INHERIT"")";
        }
    }
}
